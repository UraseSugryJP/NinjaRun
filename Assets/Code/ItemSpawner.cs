...
public class ItemSpawner : MonoBehaviour
{
    ...
    public void SpawnPathmakingItems(int[,] p, int row, float curD, float nxtD, float totalLen) {
        ...
        for (int j = 1; j <= 3; j++) {
            ...
            // --- ここだけ追加：走行距離が一周を超えても0に戻るようにする ---
            float loopD = d % totalLen;
            float t = loopD / totalLen;
            ...
            // --- 元のコードの Y+0.5f を維持 ---
            if (Physics.Raycast(sPos + Vector3.up * 10f, Vector3.down, out RaycastHit hit, 20f)) 
                sPos.y = hit.point.y + 0.5f;
